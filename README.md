...
- **Closed type:** `type Decision = Flash(Firmware) | RejectFw` where `Firmware` is closed
- **Sink / capability:** `grant flashFirmware irreversible` → `commit { flashFirmware(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/firmware-update/demo.sh
```
...
- `firmware-update_safe.eide` — the correct design.
- `firmware-update_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
